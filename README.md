...
This section is for pre-calculated meta-analysis summary statistics, generated by a program such as RAREMETALWORKER or rvtests.
...
MetaSTAAR parquet format is also supported. Specify `format: "METASTAAR"` in the record (see below for example.)

MetaSTAAR files are often in a hierarchy of directories, split up by genome chunks rather than by chromosome. You 
can use the glob pattern `**` to traverse across directories, allowing the server to index all parquet files.  

Note that MetaSTAAR parquet format is experimental. 

```YAML
- id: 4
  name: "MetaSTAAR TOPMed T2D"
  description: "MetaSTAAR score/cov files for TOPMed freeze 9 T2D samples"
  genome_build: "GRCh37"
  score_path: "data/metastaar_t2d/**/segment*.metastaar.sumstat.parquet"
  cov_path: "data/metastaar_t2d/**/segment*.metastaar.cov.parquet"
  format: "METASTAAR"
```
