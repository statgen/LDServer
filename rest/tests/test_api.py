...
def test_region_ld(client, goldstandard_ld):
    ...
    assert all(x in result['data'] for x in ['chromosome1', 'position1', 'variant1', 'chromosome2', 'position2', 'variant2', 'correlation'])
    assert result['next'] is None
    assert all(len(result['data']['correlation']) == len(result['data'][x]) for x in ['chromosome1', 'position1', 'variant1', 'chromosome2', 'position2', 'variant2', 'correlation'])
    goldstandard = goldstandard_ld('../data/region_ld_22_51241101_51241385.hap.ld')
    assert len(goldstandard) == len(result['data']['correlation'])
    for i in xrange(0, len(goldstandard)):
        ...
        assert pytest.approx(result['data']['correlation'][i], 0.00001) == goldstandard[key]
